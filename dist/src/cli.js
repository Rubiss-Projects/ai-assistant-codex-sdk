...
async function promptVar(rl, label, key, existing, required) {
    ...
    const isSensitive = loweredKey.includes("token") || loweredKey.includes("key");
    const hint = current
...
async function setup() {
    ...
    const openAiApiKey = await promptVar(rl, "OpenAI API key (leave empty to use existing Codex CLI auth)", "OPENAI_API_KEY", existing, false);
    const codexModel = await promptVar(rl, "Default Codex model", "CODEX_MODEL", existing, false);
    ...
    const lines = [
        ...
        codexModel ? `CODEX_MODEL=${codexModel}` : "# CODEX_MODEL=",
    ];
...
function update() {
    ...
    console.log("  npm install -g --install-links github:Rubiss-Projects/ai-assistant-codex-sdk");
    console.log("\nTo pin a specific version:");
    console.log("  npm install -g --install-links github:Rubiss-Projects/ai-assistant-codex-sdk#v1.0.0");
}
function help() {
