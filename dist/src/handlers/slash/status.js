export async function handleStatus(interaction, sessions) {
    try {
        ...
        const authLine = authStatus.isAuthenticated
            ? `Authenticated via \`${authStatus.authType}\` on \`${authStatus.host ?? "api.openai.com"}\``
            : `No OPENAI_API_KEY in this process. ${authStatus.statusMessage ?? ""}`;
        await interaction.editReply(`**Codex Status**\n${authLine}\nCLI version: \`${status.version}\``);
    }
    catch (err) {
