...
import { SessionManager, chunkForDiscord } from "../../codex.js";
...
export async function handleChat(
  ...
  if (interaction.channel?.isDMBased()) {
    try {
      ...
      const msg = isPathError
        ...
        : "❌ Something went wrong talking to Codex. Please try again.";
  ...
  try {
    ...
    try {
      ...
      const threadName = `Codex: ${safeName.slice(0, 50)}${safeName.length > 50 ? "…" : ""}`;
